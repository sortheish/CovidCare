...
class GraphView : GraphService.GraphView {
            ...
            COUNTRY_CASES -> {
                for (x in 0 until CovidData.graphCount.size) {
                    yValueGroup.add(
                        BarEntry(
                            x.toFloat(),
                            CovidData.graphCount[x].cases_confirmed.toFloat()
                        )
                    )
            ...
            COUNTRY_ACTIVE -> {
                for (x in 0 until CovidData.graphCount.size) {
                    yValueGroup.add(
                        BarEntry(
                            x.toFloat(),
                            CovidData.graphCount[x].case_active.toFloat()
                        )
                    )
                    barDataSet = BarDataSet(yValueGroup, "")
            ...
            COUNTRY_RECOVERED -> {
                for (x in 0 until CovidData.graphCount.size) {
                    yValueGroup.add(
                        BarEntry(
                            x.toFloat(),
                            CovidData.graphCount[x].case_recovered.toFloat()
                        )
                    )
                    barDataSet = BarDataSet(yValueGroup, "")
            ...
            COUNTRY_DEATHS -> {
                for (x in 0 until CovidData.graphCount.size) {
                    yValueGroup.add(
                        BarEntry(
                            x.toFloat(),
                            CovidData.graphCount[x].case_death.toFloat()
                        )
                    )
                    barDataSet = BarDataSet(yValueGroup, "")
    ...
    override fun getPieChart(view: View, count: Count) {
        ...
        listPie.add(
            PieEntry(
                count.case_active.toFloat(),
                view.context.getString(R.string.text_active)
            )
        )
        listPie.add(
            PieEntry(
                count.case_recovered.toFloat(),
                view.context.getString(R.string.text_recovered)
            )
        )
        listPie.add(
            PieEntry(
                count.case_death.toFloat(),
                view.context.getString(R.string.text_death)
            )
        )
